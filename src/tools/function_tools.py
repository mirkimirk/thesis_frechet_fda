...
def function_values_on_common_grid(
    funcs: list[Function],
) -> tuple[np.ndarray, np.ndarray]:
    """Evaluate a sample of functions on one shared grid.

    When the functions already share a grid, this only stacks their y-values.
    Otherwise, it mirrors the common-grid convention used by Function arithmetic.

    """
    funcs = list(funcs)
    if len(funcs) == 0:
        msg = "At least one function is required."
        raise ValueError(msg)

    x0 = funcs[0].x
    if all(len(func.x) == len(x0) and np.array_equal(func.x, x0) for func in funcs):
        return np.copy(x0), np.vstack([func.y for func in funcs])

    grid_size = max(len(func.x) for func in funcs)
    left = min(func.x[0] for func in funcs)
    right = max(func.x[-1] for func in funcs)
    x = np.linspace(left, right, grid_size)
    y = np.vstack([np.interp(x, func.x, func.y) for func in funcs])
    return x, y
...
def mean_func(funcs: list[Function]) -> Function:
    ...
    if len(funcs) == 1:
        return funcs[0]
    x, y = function_values_on_common_grid(funcs)
    return Function(x, np.mean(y, axis=0))
