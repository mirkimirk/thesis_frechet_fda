...
\end{equation}
This is done by characterizing the well-known linear regression model in terms of a
sequence of weighted means. For the Euclidean case, this characterization is given in
the following lemma:
\begin{lemma}
...
to the estimator via quadratic programming. Here,
\[
    s_{in}(p) = 1 + (X_i - \bar{X})^T \hat{\Sigma}^{-1} (p - \bar{X})
\]
denotes the empirical version of the weight function from Lemma~\ref{lemma:sequence_weights},
with $\bar{X} = \frac{1}{n} \sum_{i=1}^{n} X_i$ and
$\hat{\Sigma} = \frac{1}{n} \sum_{i=1}^{n} (X_i - \bar{X})(X_i - \bar{X})^T$
\parencite[cf.][]{PetersenMüller2019}.
\begin{algorithm}
    ...
    \For{\( l = 1, \ldots, m \)}
        \State Compute \( Q_l = \frac{1}{n} \sum_{i=1}^{n} s_{in}(p) Q_i(u_l) \)
    \EndFor
