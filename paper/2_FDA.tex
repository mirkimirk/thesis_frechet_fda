...
In fda, it is generally assumed that the functions we are dealing with live in a
\textit{Hilbert space}:
\begin{definition}[Hilbert space]
    A (real) vector space $\mathcal{H}$, endowed with an inner product $\inpr{\cdot}{\cdot}_\mathcal{H}$,
    a norm $\norm{\cdot}_\mathcal{H}$ that is induced by this inner product, and that satisfies
    completeness with respect to this norm, is called a (real) \textit{Hilbert space}.
\end{definition}
...
$d_\mathcal{H}(a, b) = \norm{a - b}_\mathcal{H}$ to measure
the distance between two elements in $\mathcal{H}$. The concept of inner product makes it
possible to evaluate the angle between two vectors in $\mathcal{H}$, which is useful for
finding orthogonal elements.
