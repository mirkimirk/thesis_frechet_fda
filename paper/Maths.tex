...
\begin{definition}[Inner Product and Inner Product Space]
    ...
    \begin{enumerate}
        ...
        \item \textbf{Positive Definiteness}: \( \inpr{t}{t}_V \geq 0 \) for all \( t \in V \), and \( \inpr{t}{t}_V = 0 \) if and only if \( t = 0 \).
    \end{enumerate}
    A real vector space \( V \) equipped with such an inner product is called an
    \textit{inner product space}.
\end{definition}
To generalize all the notions of interest for fda from Euclidean space to possibly
...
\begin{definition}[pdfs, cdfs, qfs, and qdfs]
    ...
    \begin{enumerate}
        ...
        \item The \textit{cumulative distribution function (cdf)} is given by
...
Setting the distance function to be $d_W$ in \eqref{eq:fmean} yields the
Wasserstein--Fréchet mean $f_\oplus$ \parencite[cf.][]{PetersenLiuDivani2021}. The
following lemma shows that it can be estimated by simple averaging in the space of
quantile density functions:
\begin{lemma}
    \label{lemma:wf_mean_estimator}
    The Wasserstein--Fréchet mean estimator is given by
    \begin{equation}
        \hat{f}_\oplus(x) = \frac{1}{\hat{q}_\oplus(\hat{F}_\oplus(x))},
    \end{equation}
    with $\hat{q}_\oplus = \frac{1}{n} \sum_{i=1}^{n} q_i$, and $\hat{F}_\oplus$ the cdf
    corresponding to $\hat{q}_\oplus$.
\end{lemma}
\begin{proof}
    ...
    $Q_\oplus(t) = \mathbb{E}[Q(t)]$ because the pointwise mean $\mathbb{E}[Q(t)]$ is
    the minimizer of
    \begin{equation}
    \label{eq:wf_mean}
        \begin{aligned}
    ...
    function, we use \eqref{eq:finverseqdf} to get
    $f_\oplus(x) = \frac{1}{q_\oplus(F_\oplus(x))}$, which shows that it suffices to
    estimate $q_\oplus$. Since
    $q_\oplus = \frac{\mathrm{d}Q_\oplus}{\mathrm{d}t}$, we can pass the differentiation
    inside the expectation to see
    $q_\oplus = \mathbb{E}\left[\frac{\mathrm{d}Q}{\mathrm{d}t}\right] = \mathbb{E}[q]$,
    which by the analogy principle suggests averaging the observed (or previously
    estimated) quantile densities $q_i$ to obtain an estimator $\hat{q}_\oplus$ for
    $q_\oplus$.
\end{proof}
